...
class TreeNode:
    ...
    def readTreeList(self, nums):
        ...
        while not stop:
            ...
            L_next = []
            l = len([item for item in L_current if item != None])
            for k1 in range(ind + 1, ind + 1 + 2*l):
                if k1 < len(nums):
                    if nums[k1] == None:
                        #print('Current Tree is None')
                        L_next.append(None)
                    else:
                        #print('Current Tree is not None and has value ', nums[k1])
                        L_next.append(TreeNode(nums[k1]))
                else:
                    L_next.append(None)
            j = 0
            for k2 in L_current:
    ...
    def printTreeNode(self):
        ...
        while (len([i for i in L if i != None]) != 0) and j < 15:
            ...
            # Pop off last Nones
            while len(l) != 0 and l[-1] == 'null':
                del l[-1]
            # Pop off first Nones
            while len(l) != 0 and l[0] == 'null':
                del l[0]
            if len(l) < 40:
                print('Layer: ',j,' has ',l)
            else:
...
#input3 = [3,9,20,null,null,null,7] + [null,8] + [9,null]
