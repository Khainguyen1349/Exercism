...
def readTreeList(nums: list) -> TreeNode:
    num_level = int(math.log2(len(nums) + 1))
    print('Tree has ',num_level,' levels')
    tree = TreeNode(nums[0],TreeNode(),TreeNode())
    L = [tree.left,tree.right]
...
def printTreeNode(tree: TreeNode):
    L = [tree]
    j = 1
    while len([i for i in L if i != None]) != 0:
        l = []
        Ltemp = []
        for k in L:
            if k == None:
